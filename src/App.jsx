    ...
    contents: [
      {
        ...
        parts: [
          { 
            text: "請分析這張日本收據。提取出店名(name，請務必將日文翻譯成繁體中文)、日期(date, 格式為 YYYY-MM-DD)、總金額(amountJpy, 僅數字)、最適當的類別(category, 必須是以下之一：餐飲, 交通, 購物, 住宿, 景點, 其他)、以及簡單的備註(note, 例如買了什麼)。金額請確保是日圓(JPY)。若日期包含漢字(年/月/日)請轉換為標準的 YYYY-MM-DD。" 
          },
...
export default function App() {
  ...
  const clearAll = () => {
    if(confirm('確定清除本機所有紀錄？(注意：此動作不會刪除 Google 雲端上的資料)')) {
      setExpenses([]);
    }
  };
  ...
  const handleFileChange = async (e) => {
    ...
    setIsScanning(true); setScanStatus('正在解析日本收據格式...');
    ...
    try {
      ...
      reader.onloadend = async () => {
        ...
        try {
          ...
          alert(error.message); setIsScanning(false);
        }
      };
      reader.readAsDataURL(file);
    } catch (err) {
      alert("讀取圖片失敗"); setIsScanning(false);
    }
    e.target.value = '';
  };
  ...
  const exportCSV = () => {
    ...
    const rows = sortedExpenses.map(e => [
      e.date, `"${e.name}"`, e.category, e.amountJpy, Math.round(e.amountJpy * settings.exchangeRate), `"${e.note || ''}"`
    ]);
  ...
  const renderHome = () => (
    <div className="p-4 pb-24 space-y-4 animate-in fade-in font-zen">
      ...
      <div onClick={triggerCamera} className="bg-[#fffdf8] border-2 border-dashed border-[#d4c4a8] rounded-2xl p-6 text-center cursor-pointer active:scale-95 transition-transform shadow-sm">
      ...
      <div className="bg-[#fffdf8] border border-[#d4c4a8] rounded-2xl p-4 shadow-sm">
        ...
        <button onClick={handleQuickAdd} className="w-full bg-[#c0392b] text-white py-3 rounded-xl font-bold tracking-wide active:scale-95 transition-transform shadow-sm">＋ 新增記帳</button>
      </div>
      ...
      <div className="p-4 pb-24 space-y-4 animate-in fade-in font-zen">
        <div className="bg-[#fffdf8] p-4 rounded-2xl shadow-sm border border-[#d4c4a8] mt-4">
          ...
          {(filterStartDate || filterEndDate) && (
            <button onClick={() => { setFilterStartDate(''); setFilterEndDate(''); }} className="mt-3 w-full py-2 bg-[#f0e8d8] text-[#8c7b6b] rounded-lg text-sm font-bold active:scale-95 transition-transform">
              清除區間 (顯示全部)
            </button>
          )}
        </div>
        ...
        {filteredTotalJpy === 0 ? (
          <div className="text-center text-[#8c7b6b] mt-16 bg-[#fffdf8] p-6 rounded-2xl border border-[#d4c4a8]">此區間暫無消費紀錄</div>
        ) : (
          <div className="bg-[#fffdf8] p-6 rounded-2xl shadow-sm border border-[#d4c4a8]">
            ...
            <div className="flex justify-center mb-8">
              <div className="w-48 h-48 rounded-full shadow-inner relative" style={{ background: `conic-gradient(${conicGradientStr})` }}>
            ...
            <div className="space-y-3">
              {categoryTotals.map(cat => (
                <div key={cat.name} className="flex items-center justify-between text-[0.95rem]">
                  <div className="flex items-center gap-2"><div className="w-3 h-3 rounded-full" style={{ backgroundColor: cat.color }}></div><span className="text-[#1a1209] font-medium">{cat.emoji} {cat.name}</span></div>
                  <div className="flex gap-4"><span className="font-bold text-[#1a1209]">¥{formatCurrency(cat.amount)}</span><span className="text-[#8c7b6b] w-10 text-right text-xs pt-0.5">{Math.round((cat.amount/filteredTotalJpy)*100)}%</span></div>
                </div>
              ))}
            </div>
          </div>
  ...
  const renderSettings = () => (
    <div className="p-4 pb-24 space-y-6 animate-in fade-in font-zen">
      <div className="bg-[#fffdf8] border border-[#d4c4a8] rounded-2xl p-5 mt-4">
        ...
        <button onClick={exportCSV} className="w-full bg-[#f0e8d8] border border-[#d4c4a8] text-[#1a1209] font-bold py-3 rounded-xl mb-3 shadow-sm active:scale-95 transition-transform">📤 匯出 CSV</button>
        <button onClick={clearAll} className="w-full bg-[#f0e8d8] border border-[#d4c4a8] text-[#c0392b] font-bold py-3 rounded-xl shadow-sm active:scale-95 transition-transform">🗑️ 清除所有資料</button>
      </div>
    </div>
  );
